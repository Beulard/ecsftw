...
FileStream::FileStream( const std::string& file, int mode ) {
    Open( file, mode );
}
...
void FileStream::Open( const std::string& f, int mode ) {
    file = f;
    stream.open( file.c_str(), GetMode(mode) );
    if( !stream.is_open() )
        Error( "Could not open file %s for reading", file.c_str() );
}
